        ...
        executorService.submit(() -> {
            ...
            if (scheduleJob.getStartDate().getTime() <= System.currentTimeMillis()
                ...
                if (!StringUtils.isEmpty(jobType)) {
                    ...
                    try {
                        String cache = redisClient.get(RedisConsts.EMAIL_UNABLE_SEND_LIST);
                        Set<Long> jobIdList = CommonUtils.getIdSetByCache(cache);
                        if (scheduleService.shouldExecute(scheduleJob.getId())) {
                            jobIdList.remove(scheduleJob.getId());
                            scheduleService.execute(scheduleJob.getId());
                        } else {
                            jobIdList.add(scheduleJob.getId());
                        }
                        redisClient.setex(RedisConsts.EMAIL_UNABLE_SEND_LIST, DateUtils.getSeconds(), JSONObject.toJSONString(jobIdList));
                    } catch (Exception e) {
                        e.printStackTrace();
