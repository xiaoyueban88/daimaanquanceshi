...
public class MailUtils {
    ...
    private static final String MAIL_HTML_SUF_KEY = "sufHtml";

    private static final String MAIL_IMG_PLACE = "\\$img";
    ...
    public void sendMail(MailContent mailContent, Logger customLogger) throws ServerException {
        ...
        switch (mailContent.getMailContentType()) {
            ...
            case HTML:
                ...
                String htmlContent = mailContent.getHtmlContent();
                String preContent = "";
                String sufContent = "";
                if (!StringUtils.isEmpty(htmlContent)) {
                    String[] array = htmlContent.split(MAIL_IMG_PLACE);
                    preContent = array[0];
                    if (array.length > 1) {
                        sufContent = array[1];
                    }
                }
                context.setVariable(MAIL_HTML_KEY, preContent);
                context.setVariable(MAIL_HTML_SUF_KEY, sufContent);
                mailContentTemplate = EMAIL_DEFAULT_TEMPLATE;
                break;
            case TEMPLATE:
