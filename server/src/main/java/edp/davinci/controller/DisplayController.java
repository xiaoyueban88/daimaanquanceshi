...
public class DisplayController extends BaseController {
    ...
    public ResponseEntity uploadFile(@RequestParam("backgroundImage") MultipartFile file,
                                             HttpServletRequest request) throws IOException {
        String fullUrl = fileCloudUtils.uploadFileReturnFullUrl("file" , UUID.randomUUID() +"."+
                FilenameUtils.getExtension(file.getOriginalFilename()), file.getInputStream());
        return ResponseEntity.ok(new ResultMap(tokenUtils).successAndRefreshToken(request).payload(fullUrl));
    }
}
