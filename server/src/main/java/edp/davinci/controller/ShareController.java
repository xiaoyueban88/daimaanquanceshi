...
public class ShareController extends BaseController {
    ...
    @AuthShare
    @GetMapping("/widgetInfo/{dashboardOrWidgetToken}/{widgetId}")
    public  ResponseEntity getWidgetInfo(@PathVariable String dashboardOrWidgetToken,
                                         @PathVariable Long widgetId,
                                         @ApiIgnore @CurrentUser User user) {
        if (StringUtils.isEmpty(dashboardOrWidgetToken)) {
            ResultMap resultMap = new ResultMap().fail().message("Invalid share token");
            return ResponseEntity.status(resultMap.getCode()).body(resultMap);
        }
        ShareWidget shareWidgetInfo = shareService.getShareWidgetInfo(dashboardOrWidgetToken, widgetId, user);
    ...
    public ResponseEntity getUnionDistinctValue(@Valid @RequestBody List<ViewDistinctParam> params,
        ...
        try {
            ResultMap unionDistinctValue = shareService.getUnionDistinctValue(token, params, user, request);
...
//    @PostMapping(value = "/csv/{token}", consumes = MediaType.APPLICATION_JSON_VALUE)
    ...
    @ApiOperation(value = "log tracing")
    @AuthShare
    @PostMapping(value = "/trace/{opCode}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity generateTracing(@PathVariable String opCode,
                                          @RequestBody Map<String, String> otherInfos,
                                          HttpServletRequest request) {
        ...
        if (StringUtils.isEmpty(acessToken)) {
            isPlatform = false;
            acessToken = request.getHeader(Constants.TOKEN_HEADER_STRING);
        }
        shareService.generateTracing(opCode, acessToken, otherInfos, isPlatform, request);
