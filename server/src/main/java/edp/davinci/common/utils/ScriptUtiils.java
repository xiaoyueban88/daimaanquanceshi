                ...
                for (String key : vsom.keySet()) {
                    ...
                    try {
                        if (o instanceof ScriptObjectMirror) {
                            ...
                            } else if ("ellipsisConditions".equals(key)) {
                                if (mirror.isArray()) {
                                    List<Map> conditionList = Lists.newArrayList();
                                    for (int i = 0; i < mirror.size(); i++) {
                                        Map map = (Map<String, String>) mirror.get(i + EMPTY);
                                        conditionList.add(map);
                                    }
                                    header.setEllipsisConditions(conditionList);
                                }
                            }
                        }
                        else {
