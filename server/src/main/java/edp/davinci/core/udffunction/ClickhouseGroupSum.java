 ...
 */
@Component
@UdfFunctionDiscrimination(functionName = "groupSum", sourceType = "clickhouse")
public class ClickhouseGroupSum implements IUdfFunction {
    ...
    public Pair<ExtraAggregator, ExtraAggregator> prepare(UdfFunctionDto udfFunctionDto) {
        ...
        // 生成widget层的sql片段
        String sql = new StringBuilder("sum(").append(expression).append(")")
