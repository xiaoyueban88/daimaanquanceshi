...
public interface DashboardMapper {
    ...
    @Update({"update dashboard set is_delete = 1 where id = #{id}"})
    int logicDeleteById(@Param("id") Long id);

    @Update({"update dashboard set is_delete = 1 where dashboard_portal_id = #{portalId}"})
    int logicDeleteByPortalId(@Param("portalId") Long portalId);

    @Select({"select id from dashboard where id_delete = 1"})
    ...
    @Select({"select * from dashboard where id = #{id} and is_delete = 0"})
    Dashboard getById(@Param("id") Long id);


    @Select({"select id from dashboard where dashboard_portal_id = #{portalId} and `name` = #{name} and is_delete = 0"})
    ...
    @Select({
            "select * from dashboard where dashboard_portal_id = #{portalId} and is_delete = 0 order by `index`"
    })
    ...
    @Select({
            "SELECT * FROM dashboard WHERE (parent_id = #{parentId} OR id = #{parentId}) and is_delete = 0 "
    })
    ...
    @Select({"select full_parent_id from dashboard where id = #{id} and is_delete = 0"})
    ...
    @Select({
            "select * from dashboard where type = 1 and is_delete = 0 and FIND_IN_SET(#{id},full_parent_Id)"
    })
