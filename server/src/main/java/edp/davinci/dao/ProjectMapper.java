...
public interface ProjectMapper {
    ...
    @Select({"select id from project where org_id = #{orgId} and `name` = #{name} and is_delete = 0"})
    ...
    @Select({"select * from project where id = #{id} and user_id = #{userId} and is_delete = 0"})
    ...
    @Update({"update project set is_delete = 1 where id = #{id}"})
    int logicDeleteById(@Param("id") Long id);

    @Select({"select * from project where org_id = #{orgId} and is_delete = 0"})
    ...
    @Select({"select id from project where is_delete = 1"})
