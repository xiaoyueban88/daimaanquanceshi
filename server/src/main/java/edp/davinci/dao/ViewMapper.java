...
public interface ViewMapper {
    ...
    @Select({
            ...
            "from `view` v ",
            "left join source s on s.id = v.source_id ",
            "where v.project_id = #{projectId} order by v.create_time desc"
    })
    ...
    @Select({"SELECT v.id, v.sql from view v"})
    Set<View> getAllViews();

    Set<ViewSqlInfo> getViewSqlInfo(@Param("widgetIds") Set<Long> widgetIds);
