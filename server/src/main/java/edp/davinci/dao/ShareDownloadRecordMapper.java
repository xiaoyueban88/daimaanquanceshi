...
public interface ShareDownloadRecordMapper {
    ...
    Long insertSelective(ShareDownloadRecord record);
    ...
    @Select({
            "SELECT * FROM share_download_record WHERE `uuid` = #{uuid, jdbcType=VARCHAR} and `download_type` = #{downloadType, jdbcType=SMALLINT} and create_time > DATE_FORMAT((NOW() - INTERVAL 2 DAY),'%Y%m%d') order by create_time desc limit 10"
    })
    List<ShareDownloadRecord> getShareDownloadRecordsByUuid(@Param("uuid") String uuid, @Param("downloadType") Integer downloadType);
    ...
    @Select({
            "SELECT * FROM share_download_record " +
                    "WHERE id = #{id, jdbcType=BIGINT}"
    })
    ShareDownloadRecord getById(@Param("id") Integer id);
