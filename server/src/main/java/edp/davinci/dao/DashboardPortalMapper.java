...
public interface DashboardPortalMapper {
    ...
    @Update({"update dashboard_portal set is_delete = 1 where id = #{id}"})
    int logicDeleteById(@Param("id") Long id);

    @Select({"select * from dashboard_portal where id = #{id} and is_delete = 0"})
    ...
    @Select({"select id from dashboard_portal where project_id = #{projectId} and name = #{name} and is_delete = 0"})
    Long getByNameWithProjectId(@Param("name") String name, @Param("projectId") Long projectId);

    @Select({"select * from dashboard_portal where project_id = #{projectId} and is_delete = 0"})
    ...
    @Select({
            ...
            "WHERE dp.id = #{id} and dp.is_delete = 0",
    })
    ...
    @Select({"select id from dashboard_portal where is_delete = 1"})
