...
public interface WidgetMapper {
    ...
    @Select({"select * from widget where project_id = #{projectId} and folder_id = #{folderId}"})
    List<Widget> getByFolderId(@Param("projectId") Long projectId, @Param("folderId") Long folderId);
    ...
    @Update({"update widget set `folder_id` = #{folderId} where id = #{id}"})
    ...
    @Select({"select id from widget where project_id = #{projectId} and folder_id = #{folderId} and `name` = #{name}"})
    Long getByNameWithFolderId(@Param("name") String name, @Param("folderId") Long folderId, @Param("projectId") Long projectId);

    @Select({"select * from widget where project_id = #{projectId} order by update_time desc"})
    ...
    int updateFolderIdBatch(@Param("list") List<Widget> list);
     ...
     * @return
     */
    @Select({"SELECT w.* FROM mem_dashboard_widget m LEFT JOIN widget w on w.id = m.widget_Id WHERE m.dashboard_id = #{dashboardId} and m.widget_type=0"})
    Set<Widget> getWidgetsByDashboard(@Param("dashboardId") Long dashboardId);

    List<Widget> getWidgetListByFolder(@Param("projectId") Long projectId, @Param("folderId") Long folderId);

    List<Widget> getWidgetsAndWidgetContainers(@Param("dashboardId") Long dashboardId);

    List<Widget> batchQueryWidgetsAndContainersOrderByCreateTime(@Param("projectId") Long projectId,
