...
public abstract class AbstractSheetWriter {
    ...
    protected void init(SheetContext context) throws Exception {
        ...
        myDefault.setAlignment(HorizontalAlignment.CENTER);
        myDefault.setVerticalAlignment(VerticalAlignment.CENTER);
        //常规格式
        ...
        general.setVerticalAlignment(VerticalAlignment.CENTER);
        //表头格式 粗体居中
    ...
    protected void writeLine(SheetContext context, Map<String, Object> dataMap) {
        Row row = context.getSheet().createRow(nextRowNum++);
        Map<String, ExcelHeader> headerMap = Maps.newHashMap();
        if(!CollectionUtils.isEmpty(context.getExcelHeaders())) {
            context.getExcelHeaders().forEach(header -> headerMap.put(header.getKey(), header));
        }
        for (int j = 0; j < context.getQueryColumns().size(); j++) {
            ...
            ExcelHeader excelHeader = headerMap.get(queryColumn.getName());
            // 判断是否需要显示省略字符"-"
//            if(null != excelHeader && matchEllipsis(value, excelHeader.getEllipsisConditions())
//                    && !context.getHasTemplate()) {
//                cell.setCellValue(Consts.ELLIPSIS);
//                cell.setCellStyle(myDefault);
//                continue;
//            }
            if (null != value) {
                if (value instanceof Number || "value".equals(queryColumn.getType())) {
...
//                    d = d / 10;
    ...
    private boolean matchEllipsis(Object cellValue, List<Map> conditions) {
        boolean match = false;
        if(CollectionUtils.isEmpty(conditions)) {
            return  match;
        }
        try {
            ScriptEngine engine = ScriptUtiils.getCellValueScriptEngine();
            Invocable invocable = (Invocable) engine;
            Object obj = invocable.invokeFunction("matchEllipsis", JSONArray.toJSONString(conditions), cellValue);
            match = (Boolean) obj;
        } catch (Exception e) {
            e.printStackTrace();
        }
