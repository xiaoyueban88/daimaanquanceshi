...
public class SheetWorker<T> extends AbstractSheetWriter implements Callable {
    ...
    public T call() throws Exception {
        ...
        log.info("Task ({}) -- {} -- widgetId={} start ", context.getTaskKey(), context.getName(), context.getWidgetId());
        Boolean rst = true;
        try {
            JdbcTemplate template = context.getSqlUtils().jdbcTemplate();

            if(context.getShowMode()==1) {
            ...
            CoordinateSheetWriter coordinateSheetWriter = null;
            if (context.getShowMode()==2) {
                coordinateSheetWriter = new CoordinateSheetWriter(context);
                coordinateSheetWriter.writeCoordinate(template,sql);
            }
            ...
            log.info("Task ({}) -- {} -- widgetId={} start query ", context.getTaskKey(), context.getName(), context.getWidgetId());
            if(this.context.getShowMode()==1) {
            ...
            log.error(String.format("sheet worker error,  task=%s, context=%s, widgetId=%s, error=%s",
                    context.getTaskKey(), context.toString(), context.getWidgetId(), e.getMessage()), e);
            if (context.getCustomLogger() != null) {
