...
public class ExecutorUtil {
    ...
    public static final ExecutorService WORKBOOK_BATCH_WORKERS = Executors.newFixedThreadPool(4,
            new ThreadFactoryBuilder().setNameFormat("workbook-worker-%d").setDaemon(true).build());

    public static final ExecutorService SHEET_BATCH_WORKERS = Executors.newFixedThreadPool(6,
            new ThreadFactoryBuilder().setNameFormat("sheet-worker-%d").setDaemon(true).build());

    public static final ExecutorService WORKBOOK_BATCH_TASK_WORKERS = Executors.newFixedThreadPool(2,
            new ThreadFactoryBuilder().setNameFormat("workbook-worker-%d").setDaemon(true).build());
    ...
    public static <T> Future<T> submitWorkbookBatchTask(WorkBookContext context, Logger customLogger) {
        return ExecutorUtil.submitWorkbookTask(new WorkbookBatchWorker(context), customLogger);
    }

    private static <T> Future<T> submitWorkbookTask(WorkbookBatchWorker worker, Logger customLogger) {
        printThreadPoolStatusLog(WORKBOOK_BATCH_WORKERS, "WORKBOOK_WORKERS", customLogger);
        return ExecutorUtil.WORKBOOK_BATCH_WORKERS.submit(worker);
    }
    ...
    public static <T> Future<T> submitSheetBatchTask(SheetWorker worker, Logger customLogger) {
        printThreadPoolStatusLog(SHEET_BATCH_WORKERS, "SHEET_WORKERS", customLogger);
        return ExecutorUtil.SHEET_BATCH_WORKERS.submit(worker);
    }

    public static <T> Future<T> submitSheetBatchTask(SheetContext context, Logger customLogger) {
        return ExecutorUtil.submitSheetBatchTask(new SheetWorker(context), customLogger);
    }
    ...
    public static <T> Future<T> submitWorkbookBatchTaskWorker(WorkbookBatchTaskWorker worker, Logger customLogger) {
        printThreadPoolStatusLog(WORKBOOK_BATCH_TASK_WORKERS, "WORKBOOK_WORKERS", customLogger);
        return ExecutorUtil.WORKBOOK_BATCH_TASK_WORKERS.submit(worker);
    }
}
