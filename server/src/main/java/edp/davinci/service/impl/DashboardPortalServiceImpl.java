...
public class DashboardPortalServiceImpl extends VizCommonService implements DashboardPortalService {
    ...
    public DashboardPortal createDashboardPortal(DashboardPortalCreate dashboardPortalCreate, User user) throws NotFoundException, UnAuthorizedException, ServerException {
        ...
        if (projectPermission.getVizPermission() < UserPermissionEnum.WRITE.getPermission()) {
            ...
            throw new UnAuthorizedException("you have not permission to create portal");
        }
    ...
    @Override
    @Transactional
    public void deleteDashboardPortal(Long id, User user) throws NotFoundException, UnAuthorizedException {
        DashboardPortal dashboardPortal = dashboardPortalMapper.getById(id);
        if (null == dashboardPortal) {
            log.info("dashboard portal (:{}) not found", id);
            throw new ServerException("dashboard portal not found");
        }
        ...
        if (projectPermission.getVizPermission() < UserPermissionEnum.DELETE.getPermission() ||
            ...
            throw new UnAuthorizedException("you have not permission to delete portal");
        }

        //delete dashboard_portal
        int i = dashboardPortalMapper.logicDeleteById(id);
        int j = dashboardMapper.logicDeleteByPortalId(id);
        if (i > 0 && j > 0) {
            relRolePortalMapper.deleteByProtalId(dashboardPortal.getId());
            optLogger.info("dashboaard portal( {} ) delete by user( :{}) ", dashboardPortal.toString(), user.getId());
        } else {
            throw new ServerException("delete portal error");
        }
    }
}
