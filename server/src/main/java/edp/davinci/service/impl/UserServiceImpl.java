...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Transactional
    public synchronized User regist(com.zhixue.auth.model.User currentUser) throws ServerException {
        // 用户名是否已经注册
        if (isExist(currentUser.getId(), null, null)) {
            Long idByName = userMapper.getIdByName(currentUser.getId());
            return userMapper.getById(idByName);
        }

        User user = convertToUser(currentUser, true);
        // 添加用户
        int insert = userMapper.insert(user);

        if (insert <= 0) {
            log.info("regist fail: {}", user.toString());
            throw new ServerException("regist fail: unspecified error");
        }

        // 初始用户自动移入公共组织下
        Long userId = userMapper.getIdByName(user.getUsername());
        Long publicOrgId = organizationMapper.getIdByName("public");
        //验证通过，建立关联
        RelUserOrganization rel = new RelUserOrganization(publicOrgId, userId, UserOrgRoleEnum.OWNER.getRole());
        int insertResult = relUserOrganizationMapper.insert(rel);
        if (insertResult > 0) {
            log.info("regist success: {}", user.toString());
     ...
     */
    @Override
    public List<UserBaseInfo> getUsersByKeyword(String keyword, User user, Long projectId, Boolean includeSelf) {
        Project project = projectMapper.getById(projectId);
        // 获取当前项目
        Long orgId = project == null ? null : project.getOrgId();
    ...
    /*public  static void main(String[] args){
        System.out.println(BCrypt.hashpw("iflytek123!@#", BCrypt.gensalt()));
    }*/
...
//    }
    ...
    private User convertToUser(com.zhixue.auth.model.User currentUser, boolean isActive) {
        User user = new User();
        user.setUsername(currentUser.getId());
        user.setPassword("");
        user.setEmail(StringUtils.isEmpty(currentUser.getUserMail()) || "null".equals(currentUser.getUserMail())
                ? currentUser.getId() + "@iflytek.com" : currentUser.getUserMail());
