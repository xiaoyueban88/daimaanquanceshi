...
function getWidgetFields (widgetConfig) {
    ...
    if(widgetConfig.secondaryMetrics) {
        for (var j = 0; j < widgetConfig.secondaryMetrics.length; j++) {
            var s = widgetConfig.secondaryMetrics[j]
            if (withNoAggregators) {
                s.agg = ''
            }
            secondaryMetrics.push(s)
        }
    }
    var colors = []
    if(widgetConfig.color && widgetConfig.color.items && widgetConfig.color.items.length > 0) {
        colors = widgetConfig.color.items
    }
    var tipItems = []
    if(widgetConfig.tip && widgetConfig.tip.items && widgetConfig.tip.items.length > 0) {
        tipItems = widgetConfig.tip.items
    }
    var fields = cols.concat(rows).concat(colors).concat(metrics).concat(secondaryMetrics).concat(tipItems)
    return fields
}
...
function matchEllipsis(conditionJson, cellVal) {
    var conditions = JSON.parse(conditionJson)
    var match = false
    if(conditions && conditions.length > 0) {
        for (var i=0; i < conditions.length; i++) {
            var operatorType = conditions[i].operatorType
            var conditionValues = conditions[i].conditionValues
            switch (operatorType) {
                case "between":
                    var minVal = conditionValues[0]
                    var maxVal = conditionValues[1]
                    match = (minVal <= cellVal && maxVal >= cellVal) || match
                    break;
                case "like":
                    match = (cellVal.toString().indexOf(conditionValues[0].toString()) >= 0) || match
                    break;
                case "=":
                    match = (cellVal == conditionValues[0]) || match
                    break
                case ">":
                    match = (cellVal > conditionValues[0]) || match
                    break
                case ">=":
                    ...
                    break
                case "in":
                    match = conditionValues.indexOf(cellVal) >= 0 || match
                    break
                case "<":
                    match = (cellVal < conditionValues[0]) || match
                    break
...
function getFieldsHeader(widgetConfigJson, queryVars) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        var conditions = []
        if(field.conditionStyleConfigs) {
            for (var j = 0; j< field.conditionStyleConfigs.length; j++) {
                var config = field.conditionStyleConfigs[j]
                if("only" === config.showType && "normal" === config.conditionType
                    && "iconfont" === config.type  && config.iconfont && "#icon-ellipsis" === config.iconfont.classname) {
                    conditions.push({
        ...
        var currentCell = {
            ...
            ellipsisConditions: conditions // 下载展示省略"-"条件
        }
        if (field) {
            currentCell.type = field.type
            if (field.format) {
